<script>
    ...
    if ($websites[websiteIndex].publications.length > 0) {
        // * The number of seconds since the last publication:
        const seconds = Math.floor(new Date().getTime() / 1000) - Math.floor($websites[websiteIndex].publications[$websites[websiteIndex].publications.length - 1].timestamp / 1000);
        if (seconds < 60) lastPublished = `${seconds} seconds ago`;
        else if (seconds < 3600) lastPublished = `${Math.floor(seconds / 60)} minutes ago`;
        else if (seconds < 86400) lastPublished = `${Math.floor(seconds / 3600)} hours ago`;
        else if (seconds < 2620800) lastPublished = `${Math.floor(seconds / 86400)} days ago`;
        else if (seconds < 31449600) lastPublished = `${Math.floor(seconds / 2620800)} months ago`;
        else lastPublished = `${Math.floor(seconds / 31449600)} years ago`;
    }
</script>

<button class="website" tabindex="0" role="link">
